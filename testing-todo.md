...
- [x] After a successful discovered connection, close and reopen the app: it reconnects automatically to the selected server.
- [x] After a successful manual connection, close and reopen the app: it reconnects automatically.
- [x] Disconnect from Settings, close and reopen the app: the server is remembered and reconnects automatically.
- [x] Forget server from Settings: the active session stops; reopening the app begins discovery instead of reconnecting to the old server.
- [x] Try a new server that fails to connect: the previous preferred server remains available after reopening the app.

## Lifecycle and recovery

- [x] Rotate the device while discovering and while connecting: no duplicate scan or connection occurs.
- [x] Leave the app while it is connecting: a successful connection is still remembered on the next launch.
